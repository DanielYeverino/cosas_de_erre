...
metadatos <- gen_metadatos(9)

gen_tabla <- function(no_sel, metadatos, ent_sel){
  bd %>% 
    filter(no == no_sel) %>% 
    filter(str_extract(cve_mun, "^\\d\\d") == ent_sel) %>% 
    left_join(metadatos, by = "no") %>% 
    left_join(catalogo_municipal, by = "cve_mun") %>% 
    select(no, id, Año = year, Indicador = indicador,
           Clasificacion = clasificacion, `Clave de Municipio` = cve_mun, Municipio = municipio, Valor = valor, Ponderador = ponderador)
}
