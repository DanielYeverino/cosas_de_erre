...
opciones_dimension <- indicadores$id_dimension %>% unique()
opciones_dimension <- opciones_dimension[-4]
gen_opciones_componente <- function(dimension_sel){
  meta %>% 
    select(id_dimension, id_componente) %>% 
    unique() %>% 
  ...
  tabItems(
    ...
    tabItem("tabAnalisis", 
            ...
            fluidRow(
              ...
              column(9, 
                      leafletOutput("mapa_indicadores"),
                      br(), br(),
                      plotOutput("grafica_evo_indicador")
                      )
            )
        )
  )
...
server <- function(input, output, session) {
  ...
  output$mapa_indicadores <- renderLeaflet({
    gen_mapa_indicadores(anio_sel = input$selAnio, 
                         indicador_sel = input$selIndicador)
  })
  
  output$grafica_evo_indicador <- renderPlot({
    gen_grafica_evo_indicador(indicador_sel = input$selIndicador, 
                              edo_sel = input$selEdo)
  })
